...
recursive_case:
    ...
    PUSH {A1, LR}               // Save A1 and LR to the stack
    
    // Pell(n-1)
    SUB A1, A1, #1              // A1 (n-1) = A (n) - 1
    BL pell                     // Recursive Case1 ( Pell(n-1) )

    POP {A2, LR}                // Restore A1 and LR from the stack
    
    PUSH {A1}                   // Push return value of BL pell to the stack
    
    // Pell(n-2)                
    SUB A1, A2, #2              // A1 (n-2) = A2 (n) - 2
    BL pell                     // Recursive Case1 ( Pell(n-2)) 

    POP {A2}                    // Get Pell(n-1) from the stack
